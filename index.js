	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}*,Jangan lupa save nomor owner bot ketik !creator`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			case 'creator':
                 ...
                 client.sendMessage(from, 'Nih Nomor Owner/Creator RIBASYT',MessageType.text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
				...
				case 'tts':
					...
					if (args.length < 2) return client.sendMessage(from, 'Teksnya mana', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('Teksnya kebanyakan')
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				case 'nsfwloli':
				    try {
						...
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
				...
				case 'cerpen':
					...
					break
				case 'chord':
					if (args.length < 1) return reply('judul lagu?')
					tels = body.slice(7)
				...
				case 'textscreen':
					...
					break
			    case 'joox':
				if (args.length < 1) return reply('Nama lagunya apa?')
               ...
               case 'infogempa':
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Usernamenya mana?', text, {quoted: mek})
				...
				case 'url2img':
					...
					if (args.length < 1) return reply('Tipenya apa?')
					if (!tipelist.includes(args[0])) return reply('Tipe desktop|tablet|mobile')
					if (args.length < 2) return reply('Urlnya mana?')
				...
				case 'say':
					...
					break
				case 'artinama':
					if (args.length < 1) return reply('Nama siapa?')
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ 𝘽𝙊𝙏 𝘽𝙍𝙊𝘼𝘿𝘾𝘼𝙎𝙏 ❯\n\n${body.slice(4)}`})
						}
				...
				case 'clone':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag sayang')
